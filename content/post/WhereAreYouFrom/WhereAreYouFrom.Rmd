...
#See this option? https://github.com/ropensci/ezknitr#motivation--simple-use-case
...
AUCensusData %>% filter(MasterCountry=="Undefined" & CensusYear==2011) %>% group_by(CensusCountry) %>% summarise(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
#can see no single country jumps out. 

#check Undefined over all years
AUCensusData %>% filter(MasterCountry=="Undefined") %>% group_by(CensusCountry) %>% summarise(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
...
ctry_list <- AUCensusData %>% group_by(MasterCountry) %>% summarise(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
...
AUCensusData_plot %>% filter(plot_ctry=="Other") %>% group_by(MasterCountry) %>% summarise(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
...
factororder <- AUCensusData_plot %>% filter(year==2011) %>% group_by(country) %>% summarise(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
