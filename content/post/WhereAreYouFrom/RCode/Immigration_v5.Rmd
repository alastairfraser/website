...
AU_Crosswalk <- select(AU_Crosswalk,-X__1)
...
temp <- temp %>% select("X__1","X__19") %>% filter(!is.na(temp$"X__1"))
temp$X__19 <- as.numeric(temp$X__19)
names(temp) <- c("CensusCountry","ppl")
temp$CensusYear <- year
AU_FullCensusData <- rbind(AU_FullCensusData,temp)
```
...
AUCensusData %>% filter(MasterCountry=="Undefined" & CensusYear==2011) %>% group_by(CensusCountry) %>% summarize(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
#can see no single country jumps out. 

#check Undefined over all years
AUCensusData %>% filter(MasterCountry=="Undefined") %>% group_by(CensusCountry) %>% summarize(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
...
ctry_list <- AUCensusData %>% group_by(MasterCountry) %>% summarize(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
...
AUCensusData_plot %>% filter(plot_ctry=="Other") %>% group_by(MasterCountry) %>% summarize(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
#Undefined is a *large* share... why? What are these people? 

#View(AUCensusData_plot %>% filter(plot_ctry=="Other") %>% group_by(MasterCountry) %>% summarize(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl))
...
factororder <- AUCensusData_plot %>% filter(year==2011) %>% group_by(country) %>% summarize(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)

#factororder <- AUCensusData_plot %>% group_by(country) %>% summarize(sum_ppl=sum(ppl)) %>% arrange(-sum_ppl)
...
ggplot(subset(AUCensusData_plot,country!="Total" & country!="Australia"),aes(x=year,y=share,fill = factor(country,levels=factororder))) + geom_area() + theme_bw() + guides(fill=guide_legend("Region of birth")) +
  xlab(NULL) + scale_y_continuous(breaks=seq(0,0.25,0.05),name="Share of Australian population",labels = scales::percent_format(accuracy = 1))
...
ggplot(subset(AUCensusData_plot,country!="Total" & country!="Australia",select=-share),aes(x=year,y=immig_share,fill = factor(country,levels=factororder))) + geom_area() + theme_bw() + guides(fill=guide_legend("Region of birth")) +
  xlab(NULL) + scale_y_continuous(breaks=seq(0,1,0.1),name="Share of foreign-born population",labels = scales::percent_format(accuracy = 1))
...
CDN_Data <- CDN_Crosswalk %>% select(-X__1,-MasterCountry)
