---
title: "Australian and Canadian Greenhouse Gas Emissions - update"
output:
  html_document:
    df_print: paged
    fig_width: 7
...
The conversation around greenhouse gas emissions in Australia differs from that in Canada. In Australia there is much attention on coal generation and renewable energy, where as in Canada the focus is on oil and gas, and in particular the oil sands and export pipelines. To get a sense of where emissions come from in each country, in this post I break down and compare the sources.
...
CDN_IPCC_def <- read_excel("Data/AUCDN_GHGBreakdown/CanadaAU_SectorCrosswalk.xlsx",sheet = "CDN_IPCC",range = "A3:D85")

# Import 2018 CDN NIR IPCC data
CDN_IPCC_Data <- read_excel("Data/AUCDN_GHGBreakdown/2018NIR_IPCC_Tables_CA_1990-2016.xlsx",sheet = "Summary",range = "A5:AF89")
...
Sector_def <- read_excel("Data/AUCDN_GHGBreakdown/CanadaAU_SectorCrosswalk.xlsx",sheet = "List of sectors")
...
AU_SectorDef <- read_excel("Data/AUCDN_GHGBreakdown/CanadaAU_SectorCrosswalk.xlsx",sheet = "AU_IPCC",range = "A2:G91")
...
for (YearName in c(1990:2016)) {
  ...
  temp <- read_csv(paste0("Data/AUCDN_GHGBreakdown/AGEISbyYear/AU",YearName,".csv"),col_names = TRUE)[ ,1:2]
...
for (ProvName in ProvinceList) {
  ...
  print(filename)
  temp <- read_excel(paste0("Data/AUCDN_GHGBreakdown/C-Tables-IPCC-Sector-Provinces-Territories/",filename),sheet="Summary",skip = 4)
...
filenames <- list.files(path = "Data/AUCDN_GHGBreakdown/AGEISbyState", pattern = NULL, all.files = FALSE,
           full.names = FALSE, recursive = FALSE,
           ignore.case = FALSE, include.dirs = FALSE, no.. = FALSE)

AUStateGHGs <- NULL
for (filename in filenames) {
  temp <- read_csv(paste0("Data/AUCDN_GHGBreakdown/AGEISbyState/",filename),n_max=1,col_names=TRUE)
