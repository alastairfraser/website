...
tempdata$date <- as.Date(tempdata$date, origin="1899-12-30")
rentdata <- rbind(rentdata,tempdata)
tempdata <- read_xlsx("RentData/RentalBond_Lodgements_Year_2020.xlsx",skip=1,col_names=TRUE) %>% rename(date="Lodgement Date")
tempdata$date <- as.Date(tempdata$date, origin="1899-12-30")
rentdata <- rbind(rentdata,tempdata)
tempdata <- read_xlsx("RentData/RentalBond_Lodgements_Year_2021.xlsx",skip=1,col_names=TRUE) %>% rename(date="Lodgement Date")
tempdata$date <- as.Date(tempdata$date, origin="1899-12-30")
rentdata <- rbind(rentdata,tempdata)

#montly data for 2022
#loop over date names
#months <- c("January","February","March","April","May","June","July","August","September","October","November","December")
months <- c("January")
for (month in months) {
  tempdata <- read_xlsx(paste0("RentData/RentalBond_Lodgements_",month,"_2022.xlsx"),skip=1,col_names=TRUE) %>% rename(date="Lodgement Date")
  tempdata$date <- as.Date(tempdata$date, origin="1899-12-30")
  rentdata <- rbind(rentdata,tempdata)
}
...
newdates <- tibble(date=seq(as.Date("2016-01-01"), by = "day", length.out = 3000)) %>% filter(date<as.Date("2022-02-01"))
...
plotdata <- plotdata %>% filter((date < as.Date("2022-02-01")) & (date >= as.Date("2019-01-01"))) %>% mutate(year=year(date))

#shift 2019 dates forward by a year so they can be plotted together, and shift 2021 dates back to 2020 dates and shift 2022 dates back to 2020 dates
plotdata$date[plotdata$date < as.Date("2020-01-01")] <- plotdata$date[plotdata$date < as.Date("2020-01-01")] + years(1)
plotdata$date[plotdata$date >= as.Date("2021-01-01") & plotdata$date<as.Date("2022-01-01")] <- plotdata$date[plotdata$date >= as.Date("2021-01-01") & plotdata$date<as.Date("2022-01-01")] - years(1)
plotdata$date[plotdata$date >= as.Date("2022-01-01") & plotdata$date<as.Date("2023-01-01")] <- plotdata$date[plotdata$date >= as.Date("2022-01-01") & plotdata$date<as.Date("2023-01-01")] - years(2)
...
ggplot(subset(plotdata,group=="All"),aes(x=date,y=ma7,color=year)) + geom_line(size=0.8) + 
  ...
  scale_color_manual(values = c("2019" = "grey50", "2020" = "red2", "2021" = "darkorchid4", "2022" = "darkorchid1")) + 
...
#plotdata <- rentdata %>% filter((date < as.Date("2025-12-01")) & (date >= as.Date("2019-01-01"))) %>% mutate(year=year(date))
plotdata <- rentdata %>% filter((date >= as.Date("2019-01-01"))) %>% mutate(year=year(date))
#plotdata <- rentdata %>% mutate(year=year(date))
...
#plotdata$date[plotdata$date >= as.Date("2022-01-01")] <- plotdata$date[plotdata$date >= as.Date("2022-01-01")] - dyears(1)
